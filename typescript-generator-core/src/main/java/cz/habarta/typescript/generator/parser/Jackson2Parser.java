...
public class Jackson2Parser extends ModelParser {
	...
	public Jackson2Parser(Settings settings, TypeProcessor typeProcessor, boolean useJaxbAnnotations) {
		...
		if (!settings.mixins.isEmpty()) {
			for (Entry<Class<?>, Set<Class<?>>> mixinEntry : settings.mixins.entrySet()) {
				for (Class<?> mixin : mixinEntry.getValue()) {
					objectMapper.addMixIn(mixinEntry.getKey(), mixin);
				}
			}
		}
	...
	protected BeanModel parseBean(SourceType<Class<?>> sourceClass) {
		...
		if (beanHelper != null) {
			for (BeanPropertyWriter beanPropertyWriter : beanHelper.getProperties()) {
				...
				properties.add(processTypeAndCreateProperty(beanPropertyWriter.getName(), propertyType, optional,
						sourceClass.type, originalMember, pullProperties));
			}
		}
		...
		return new BeanModel(sourceClass.type, superclass, taggedUnionClasses, discriminantProperty,
				discriminantLiteral, interfaces, properties, null);
	}
	...
	private String getDiscriminantPropertyName(JsonTypeInfo jsonTypeInfo) {
		return jsonTypeInfo.property().isEmpty() ? jsonTypeInfo.use().getDefaultPropertyName()
	...
	private String getTypeName(final Class<?> cls) {
		...
		final JsonSubTypes jsonSubTypes = getAnnotationRecursive(cls, JsonSubTypes.class,
				new Predicate<JsonSubTypes>() {
					@Override
	...
	private BeanHelper getBeanHelper(Class<?> beanClass) {
		...
		try {
			final DefaultSerializerProvider.Impl serializerProvider1 = (DefaultSerializerProvider.Impl) objectMapper
					.getSerializerProvider();
			final DefaultSerializerProvider.Impl serializerProvider2 = serializerProvider1
			...
			final JsonSerializer<?> jsonSerializer = BeanSerializerFactory.instance
					.createSerializer(serializerProvider2, simpleType);
			...
			if (jsonSerializer instanceof BeanSerializer) {
				...
				if (settings.displaySerializerWarning) {
					System.out.println(String.format("Warning: Unknown serializer '%s' for class '%s'",
							jsonSerializerName, beanClass));
				}
				return null;
			}
	...
	protected EnumModel<?> parseEnum(SourceType<Class<?>> sourceClass) {
		...
		final boolean isNumberBased = jsonFormat != null
				&& (jsonFormat.shape() == JsonFormat.Shape.NUMBER || jsonFormat.shape() == JsonFormat.Shape.NUMBER_FLOAT
